<x-layout title="Edit Word List">
    <div class="min-h-screen bg-darker">
        <div class="flex justify-between items-center mx-32 py-8">
            <h1 class="text-2xl text-white font-bold mb-4">Edit list</h1>
            <button type="submit" form="edit-list-form" class="bg-green-500 text-white px-4 py-2 rounded">Update List</button>
        </div>
        <form id="edit-list-form" action="{{ route('lists.update', $wordList) }}" method="POST" class="space-y-4 mx-32">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <input type="text" name="title" placeholder="List Title" required value="{{ $wordList->title }}"
                       class="w-full bg-lighter text-white p-6 rounded-xl border-none placeholder-white"/>
            </div>
            <div class="space-y-4" x-data="{
                ...
                <template x-for="(word, index) in words" :key="index">
                    <div class="flex items-center justify-between bg-lighter p-5 rounded-2xl">
                        <div class="flex-grow">
                            <input type="text" :name="'words['+index+'][woord]'" x-model="word.woord" placeholder="Woord" required
                                   class="w-full bg-transparent border-none text-white placeholder-white"/>
                        </div>
                        <div class="flex-grow ml-4">
                            <input type="text" :name="'words['+index+'][vertaling]'" x-model="word.vertaling" placeholder="Vertaling" required
                                   class="w-full bg-transparent border-none text-white placeholder-white"/>
                        </div>
                        <button type="button" @click="words.splice(index, 1); wordsCount--" class="ml-4 text-white">
                ...
                <div class="flex items-center justify-center bg-primary p-6 rounded-2xl cursor-pointer"
