<x-layout title="Create New List">
    <div class="min-h-screen bg-darker">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mx-4 md:mx-8 lg:mx-32 py-6 md:py-8 space-y-4 sm:space-y-0">
            <h1 class="text-xl md:text-2xl text-white font-bold translate-text" data-en="Create new list" data-nl="Nieuwe lijst maken">Create new list</h1>
            <button type="submit" form="create-list-form" class="bg-green-500 text-white px-4 py-2 rounded translate-text w-full sm:w-auto" data-en="Create" data-nl="Maken">Create</button>
        </div>
        <form id="create-list-form" action="{{ route('lists.store') }}" method="POST" class="space-y-4 mx-4 md:mx-8 lg:mx-32">
            @csrf
            <div class="mb-4">
                <input type="text" name="title" placeholder="{{ __('List Title') }}" required
